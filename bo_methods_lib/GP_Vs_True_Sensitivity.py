...
def Compare_GP_True_Movie(all_data, X_space, Xexp, Yexp, true_model_coefficients, true_p, obj, Case_Study, bounds_p, skip_param_types = 0, set_lengthscale = None, train_iter = 300, noise_std = 0.1, verbose = False, DateTime = None, save_figure= True, eval_Train = False):  
    """
...
#         print(len(eval_p))
    ...
    for i in range(len(eval_p_base)):   
        # Evaluate at the lower bound for each theta and add +10% per iter to the upper bound of theta
        percentiles = np.linspace(0,1.0,11)
        eval_p = eval_p_base.clone()
        for j in range(len(percentiles)):
            lower_theta = bounds_p[0,i]
            upper_theta = bounds_p[1,i]
            eval_p[i] = lower_theta+(upper_theta - lower_theta)*percentiles[j]
