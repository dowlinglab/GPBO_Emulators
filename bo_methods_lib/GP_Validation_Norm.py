...
#         print(data_test[0,1:-1])
    ...
    #           #Emulator always set to true for normalization for CS1 because training data always includes x values
                train_p[:,-m:] = normalize_x(Xexp, train_p[:,-m:], norm, scaler_x)[0]
                train_p[:,0:-m] = normalize_p_data(train_p, m, True, norm, scaler_theta)
                test_p[:,-m:] = normalize_x(Xexp, test_p[:,-m:], norm, scaler_x)[0]
                test_p[:,0:-m] = normalize_p_data(test_p, m, True, norm, scaler_theta)
            else:
                train_p = normalize_p_data(train_p, m, emulator, norm, scaler_theta)
                test_p = normalize_p_data(test_p, m, emulator, norm, scaler_theta)
