...
class Expected_Improvement():  
    ...
    def type_2(self, method):
        ...
        for i in range(num_thetas): #1 ei per theta and also 1 sse per theta   
            ...
            #Append theta index to row_data
            row_data['theta_idx'] = i
    ...
    def __calc_ei_emulator(self, gp_mean, gp_var, y_target): #Will need obj toggle soon
        ...
        # row_data = pd.DataFrame([[bound_lower, bound_upper, norm.cdf(bound_lower), norm.cdf(bound_upper), 
        #                           ei_eta_lower, ei_eta_upper, ei_term3_psi_lower, ei_term3_psi_upper,
        #                           ei_term1, ei_term2, ei_term3, ei, ei_temp]], columns=columns)
        # row_data_full = row_data.apply(lambda col: col.explode(), axis=1).reset_index(drop=True)

        return ei_temp, row_data_full
    ...
    def __calc_ei_log_emulator(self, gp_mean, gp_var, y_target):
        ...
        # row_data = pd.DataFrame([[self.best_error, bound_lower, bound_upper, ei_term_1, ei_term_2, ei, 
    ...
    def __calc_ei_sparse(self, gp_mean, gp_var, y_target):
        ...
        # row_data = pd.DataFrame([[self.best_error, sse_temp, min_list, ei_temp]], columns=columns)
        # row_data_full = row_data.apply(lambda col: col.explode(), axis=1).reset_index(drop=True)
        ...
        #If not, continue the algorithm normally
            ...
            if neg_ei == False:
                ...
                if self.method.emulator == False:
                    ...
                    ei_output = -1*self.gp_emulator.eval_ei_cand(self.exp_data, self.ep_bias, best_error, self.method)
                    obj = -1*ei_output[0]
                    print(obj)
