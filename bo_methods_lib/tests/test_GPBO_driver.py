...
def test_run_bo_restarts(method):
    ...
    val_data = simulator.gen_sim_data(
        num_theta_data_val, num_x_data, gen_meth_theta_val, gen_meth_x, sep_fact, True
    )
    ...
    assert (
        isinstance(one_run_gp_results.max_ei_details_df, pd.DataFrame)
        or one_run_gp_results.max_ei_details_df is None
    )
    ...
    if method.emulator == False:
        assert all(
            isinstance(var, Type_1_GP_Emulator)
            for var in one_run_gp_results.list_gp_emulator_class
        )
    else:
        assert all(
            isinstance(var, Type_2_GP_Emulator)
            for var in one_run_gp_results.list_gp_emulator_class
        )
...
# method expected value
create_heat_map_param_data_list = [
    method1,
    method2,
    method3,
    method4,
...
# method expected value
create_data_instance_from_theta_list = [
    method1,
    method2,
    method3,
    method4,
