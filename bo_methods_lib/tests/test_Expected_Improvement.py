...
type_1_list = [
    [np.array([1]), np.diag(np.array([0.05**2])), 0.5, 3.737e-26],
    [np.array([1]), np.diag(np.array([0])), 0.5, 0],
    [np.array([0.4]), np.diag(np.array([0.01**2])), 0.5, 0.1],  # check me
    [np.array([0.5]), np.diag(np.array([0.5**2])), 0.2, 0.084336],
]  # Check me
...
def test_type_1_list(gp_mean, gp_var, best_error, ei_expected):
    ...
    acq_func = Expected_Improvement(
        ep_bias, gp_mean, gp_var, exp_data, best_error_metrics, seed
    )
...
@pytest.mark.parametrize(
    "ep_bias, gp_mean, gp_var, exp_data, best_error, method, depth", ei_init_list
)
def test_ei_init_err_list(
    ep_bias, gp_mean, gp_var, exp_data, best_error, method, depth
):
    with pytest.raises((AssertionError, AttributeError, ValueError)):
        best_error_metrics = tuple([best_error, np.zeros(5), None])
        acq_func = Expected_Improvement(
            ep_bias, gp_mean, gp_var, exp_data, best_error_metrics, seed, depth
        )
...
type_2_list = [
    [
        np.array([-14, -3, 0, 1, 6]),
        np.diag(np.ones(5) * 0.05**2),
        0.2,
        GPBO_Methods(Method_name_enum(3)),
        0.98698,
    ],
    [
        np.array([-14, -3, 0, 1, 6]),
        np.diag(np.ones(5) * 0.04**2),
        0.5,
        GPBO_Methods(Method_name_enum(4)),
        34.756136,
    ],
    [
        np.array([-14, -3, 0, 1, 6]),
...
@pytest.mark.parametrize(
    "gp_mean, gp_var, best_error, method, ei_expected", type_2_list
)
...
## method, depth
type_2_err_list = [
    [GPBO_Methods(Method_name_enum(5)), None],
