...
@pytest.mark.parametrize(
    "meth_id, meth_name_e, emulator_e, obj_e, sparse_grid_e, monte_carlo_e",
    GPBO_Methods_array,
)
def test_GPBO_Methods(
    meth_id, meth_name_e, emulator_e, obj_e, sparse_grid_e, monte_carlo_e
):
    method = GPBO_Methods(meth_id)
    assert (
        method.method_name.name == meth_name_e
        and method.emulator == emulator_e
        and method.obj.name == obj_e
        and method.sparse_grid == sparse_grid_e
        and method.mc == monte_carlo_e
    )
