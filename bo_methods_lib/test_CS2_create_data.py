...
from .CS2_create_data import calc_muller, calc_y_exp, create_sse_data, create_y_data, gen_y_Theta_GP, eval_GP_emulator_BE, make_next_point
...
def test_calc_muller(X_val, Mul_pred):
    mul_pot = calc_muller(X_val, Constants, noise = 0)
...
def test_create_y_data():
    y_data = create_y_data(param_space, Constants, minima, skip_params, noise_std, noise_mean=0,random_seed=9)
    assert pytest.approx(expected_y_data, abs = 1e-2) == y_data, "Minimum Ys should be gnerated when Theta_True is used for param values"
...
def test_eval_GP_emulator_BE(): 
    #Note: Need to add test for norm_scalers w/parametrization once normalization code has tests
    BE = eval_GP_emulator_BE(Xexp, Yexp, train_p, Constants, True, "obj", skip_params)
    assert pytest.approx(np.array([0]), abs = 1e-2) == BE.flatten(), "Best Error should be 0 when Theta_True is in train_p" 

train_p_org_std = np.array([[-1,-1,-6.5,0.7, 0,0,11,0.6],
                            [-1,-1,-6.5,0.7, 0,0,11,0.6],
                            [-1,-1,-6.5,0.7, 0,0,11,0.6]])
train_p_org_emul = np.array([[-1,-1,-6.5,0.7, 0,0,11,0.6,-0.558,1.442],
                            [-1,-1,-6.5,0.7, 0,0,11,0.6,-0.050,0.467],
                            [-1,-1,-6.5,0.7, 0,0,11,0.6, 0.623,0.028]])
train_y_org_std = np.array([0.001, 0.001, 0.001])
train_y_org_std_log = np.log(np.array([0.001,0.001,0.001]))
train_y_org_emul = np.array([-146.6995, -80.7677, -108.1667])
...
def test_make_next_point(emulator, obj, train_p_org, train_y_org, n_mult):
    train_p_after, train_y_after = make_next_point(train_p_org, train_y_org, theta_b, Xexp, Yexp, emulator, Constants, obj, d, skip_params, noise_std, norm_scalers = None)
