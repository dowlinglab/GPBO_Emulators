...
for val in [1, 11]:
    ...
    for mode in modes:
        ...
        dir_base = analyzer.study_results_dir
        ...
        if save_figs is True:
            ...
            z_choices_sort = sorted(
                z_choices, key=lambda x: ("sse_sim", "sse_mean", "sse_var", "acq").index(x)
            )
            ...
            with imageio.get_writer(
                gif_path, mode="I", fps=0.3
            ) as writer:  # Note. For gif use duration instead of fps
                # For each file
                for filename in filenames:
                    ...
                    if filename == filenames[0]:
                        ...
                        new_shape = (
                            np.ceil(shape[0] / 16) * 16,
                            np.ceil(shape[1] / 16) * 16,
                            shape[2],
                        )
