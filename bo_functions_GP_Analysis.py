...
def LOO_Plots_2_Input(model, likelihood, Xexp, noise_std, run, obj, verbose = False, save_fig=False, runs = 1, DateTime=None, test_p = None, LOO = True, LSO = False):
    """
    ...
    Returns:
    --------
        Theta: ndarray, tested theta value
        SSE_GP_Analy: float, The predicted SSE
    """
    #Create LOO Plots
...
#     print(eval_point)
    GP_mean = calc_GP_outputs(model, likelihood, eval_point[0:1])[3]
    GP_stdev = calc_GP_outputs(model, likelihood, eval_point[0:1])[1]

    if obj == "obj":
        SSE_GP_Analy = GP_mean
    else:
        SSE_GP_Analy = np.exp(GP_mean)
...
#             test_y[k] = calc_GP_outputs(model, likelihood, eval_point[0:1])[3]
    ...
    for k in range(len(X_space)):
        ...
        y_sim[k] = create_y_data(sim_point[0])
    
    test_y = calc_y_exp(calc_eval_point, Xexp, noise_std, noise_mean=0,random_seed=6)
        
    Theta = np.array([point[0:2]])[0]
#     y_mean = gen_y_Theta_GP(X_exp, Theta)
    
    #How to do GP Analysis w/ 2-Input?
#     if verbose == True:
#         print("Showing X/Y Plot for Theta = ",Theta , "with Xexp =", Xexp)
#         print("SSE_Total is", SSE_GP_Analy)
         
#         plot_2GP_performance(X_space, y_sim, GP_mean, GP_stdev, Theta, Xexp, test_p = test_p, test_y = test_y, verbose=verbose)
#     print(Theta)    
    return Theta, SSE_GP_Analy
