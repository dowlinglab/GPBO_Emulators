...
class ProjectGPBO_nonoise(FlowProject):
    def __init__(self):
        super().__init__()
        current_path = Path(os.getcwd()).absolute()


@ProjectGPBO_nonoise.label
def data_exists(job):
    "Confirm when data exists"
    import numpy as np
...
@ProjectGPBO_nonoise.post(lambda job: 'finished' in job.doc)
@ProjectGPBO_nonoise.operation(with_job=True)  # directives={"omp_num_threads": 12},
def run_ep_or_sf_exp(job):
    ...
    simulator = simulator_helper_test_fxns(
        job.sp.cs_name_val, job.sp.noise_mean, job.sp.noise_std, job.sp.sim_seed
    )
    ...
    exp_data = simulator.gen_exp_data(job.sp.num_x_data, gen_meth_x, x_vals, job.sp.noise_std_pct)
    # Set simulator noise_std artifically as 1% of y_exp median (So that noise will be set rather than trained)
    # simulator.noise_std = np.abs(np.median(exp_data.y_vals)) * job.sp.noise_std_pct #Manually set noise std
    ...
    if job.sp.num_val_pts > 0:
        ...
        val_sse_data = simulator.sim_data_to_sse_sim_data(
            method, val_data, exp_data, job.sp.sep_fact, y_to_sse
        )
    ...
    # Get results
    gpbo_res_simple, gpbo_res_GP = driver.run_bo_restarts(job)
