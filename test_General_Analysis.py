...
criteria_dict2 = {"cs_name_val": 1, "gp_package": "gpflow", "meth_name_val": 1}
criteria_dict3 = {"cs_name_val": 13, "gp_package": "gpflow", "meth_name_val": 1}
criteria_dict_fail = {"cs_name_val": 1, "gp_pack": "gpflow", "meth_name_val": 1}
...
make_dir_name_list = [
    [
        criteria_dict,
        False,
        "Results/cs_name_val_1/gp_package_gpflow/meth_name_val_in_1_2_3_4_5_6_7",
    ],
    [
        criteria_dict,
        True,
        "cs_name_val_1/gp_package_gpflow/meth_name_val_in_1_2_3_4_5_6_7",
    ],
...
get_jobs_from_criteria_list = [[criteria_dict, 7], [criteria_dict2, 1]]
...
get_df_all_jobs_list = [[criteria_dict, 7], [criteria_dict2, 1]]
...
get_bmm_data_list = [[criteria_dict, 7], [criteria_dict2, 1]]
...
def test_get_bmm_data(crit_dict, exp_jobs):
    analyzer = General_Analysis(crit_dict, project, save_csv)
    fxns = [
        analyzer.get_best_data(),
        analyzer.get_median_data(),
        analyzer.get_mean_data(),
    ]
    for fxn in fxns:
...
def test_analyze_obj_vals(crit_dict, z_choices):
    ...
    for job in jobs:
        ...
        assert data.shape[0] == min(
            sp_data["bo_run_tot"] - sp_data["bo_run_num"] + 1, sp_data["bo_runs_in_job"]
        )
...
analyze_obj_vals_err_list = [
    [criteria_dict, "se", "test"],
...
def test_analyze_obj_vals_err(crit_dict, z_choices, job_val):
    ...
    with pytest.raises((AssertionError, AttributeError, ValueError, TypeError)):
        for job in jobs:
            data, data_names, data_true, sp_data = analyzer.analyze_obj_vals(
                job, z_choices
            )
...
def test_analyze_thetas_err(crit_dict, z_choices, job_val):
    ...
    with pytest.raises((AssertionError, AttributeError, TypeError)):
        for job in jobs:
            data, data_names, data_true, sp_data = analyzer.analyze_thetas(
                job, z_choices
            )
...
def test_analyze_pp_data(crit_dict):
    ...
    for job, run_num, bo_iter in zip(
        job_list_best, df_best["Run Number"], df_best["BO Iter"]
    ):
        test_data = analyzer.analyze_parity_plot_data(job, run_num, bo_iter)
        assert isinstance(test_data, Data)
        assert all(
            value is not None
            for value in [test_data.x_vals, test_data.y_vals, test_data.theta_vals]
        )


analyze_pp_data_err_list = [
    ["jobs", "test", "test"],
    [None, "test", "test"],
    ["jobs", "test", "test"],
    ["test", "run", "test"],
...
def test_analyze_pp_data_err(job_val, run, iter):
    ...
    with pytest.raises((AssertionError, AttributeError, ValueError, TypeError)):
        for job, run_num, bo_iter in zip(
            jobs, df_best["Run Number"], df_best["BO Iter"]
        ):
            if job_val != "test":
                job = job_val
            if run != "test":
...
def test_analyze_heat_maps_data(crit_dict, get_ei):
    ...
    try:
        numbers = [
            float(num) for num in string_val.replace("[", "").replace("]", "").split()
        ]
    except:
    ...
    for job, run_num, bo_iter in zip(
        ...
        for pair_id in range(pairs):
            all_data, test_mesh, param_info_dict, sp_data = analyzer.analyze_heat_maps(
                job, run_num, bo_iter, pair_id, get_ei
            )
            ...
            assert all(
                value is not None
                for value in [all_data, test_mesh, param_info_dict, sp_data]
            )
        ...
        for pair_id in range(pairs):
            ...
            for job, run_num, bo_iter in zip(
                ...
                if ei_val != "test":
                    ...
                    get_ei = ei_val
                all_data, test_mesh, param_info_dict, sp_data = analyze_heat_maps(
                    job, run_num, bo_iter, pair_id, get_ei=True
                )
